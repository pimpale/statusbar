...
impl<'a, Message, Theme, Renderer> AdvancedTextInput<'a, Message, Theme, Renderer>
where
    Message: Clone,
    Theme: Catalog,
    Renderer: text::Renderer,
...
impl<'a, Message, Theme, Renderer> Widget<Message, Theme, Renderer>
    for AdvancedTextInput<'a, Message, Theme, Renderer>
where
    Message: Clone,
    Theme: Catalog,
    Renderer: text::Renderer,
        ...
        match event {
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                ...
                if let Some(on_focus) = self.on_focus.clone() {
                    if state.is_focused.is_none() && click_position.is_some() {
                        shell.publish(on_focus);
                    }
                }
                if let Some(on_unfocus) = self.on_unfocus.clone() {
                    if state.is_focused.is_some() && click_position.is_none() {
...
impl<'a, Message, Theme, Renderer> From<AdvancedTextInput<'a, Message, Theme, Renderer>>
...
{
    fn from(
        text_input: AdvancedTextInput<'a, Message, Theme, Renderer>,
    ...
    Renderer::Theme: StyleSheet,
{
    ...
    padding: Padding,
    size: Option<f32>,
    line_height: iced_widget::text::LineHeight,
    on_input: Option<Box<dyn Fn(String) -> Message + 'a>>,
    on_paste: Option<Box<dyn Fn(String) -> Message + 'a>>,
    on_submit: Option<Message>,
    on_focus: Option<Message>,
